...
-- chọn ra các sản phầm có giá lớn hơn trung bình 
...
--chọn ra id nhân viên có số lần bán hàng nhiều nhất
...
Create Function CountProductByBrandId(
	@brand_id int
)
Returns int
As
...
Select dbo.CountProductByBrandId(1) as [CountProductByBrandId];

Create Function GetAllProductByBrandId(
	@brand_id int
)
Returns Table
As
...
Select * from GetAllProductByBrandId(1);
...
CREATE PROCEDURE uspFinProducts (
...
AS
	BEGIN 
		...
		FROM production.products
		WHERE list_price BETWEEN @min_list_price AND @max_list_price
